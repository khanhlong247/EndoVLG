import os
import io
import requests
import json
from datasets import load_dataset
from tqdm import tqdm
from concurrent.futures import ThreadPoolExecutor
from PIL import Image
from requests.adapters import HTTPAdapter
from urllib3.util.retry import Retry

# --- NEW CONFIGURATION ---
CACHE_DIR = "./kvasir_vqa_x1_data"
OUTPUT_IMAGES_DIR = "./kvasir_vqa_ready/images"
NUM_THREADS = 2
TIMEOUT_VAL = (10, 120)

os.makedirs(OUTPUT_IMAGES_DIR, exist_ok=True)

session = requests.Session()
retries = Retry(total=5, backoff_factor=1, status_forcelist=[500, 502, 503, 504])
session.mount('https://', HTTPAdapter(max_retries=retries))

print("Loading dataset from cache...")
dataset = load_dataset("SimulaMet/Kvasir-VQA-x1", cache_dir=CACHE_DIR)
data_subset = dataset['train']

# Filter unique images
unique_images = {}
for item in data_subset:
    if item['img_id'] not in unique_images:
        unique_images[item['img_id']] = item
unique_data_list = list(unique_images.values())

def is_valid_image(path):
    if not os.path.exists(path) or os.path.getsize(path) < 100:
        return False
    try:
        with Image.open(path) as img:
            img.verify()
        return True
    except:
        return False

def save_and_verify(item):
    img_id = item['img_id']
    img_data = item['image']
    save_path = os.path.join(OUTPUT_IMAGES_DIR, f"{img_id}.jpg")

    # Only process images that don't exist yet or are corrupted
    if os.path.exists(save_path) and is_valid_image(save_path):
        return None

    try:
        # Handle the string (URL/Path) case - the source of Timeout errors
        if isinstance(img_data, str):
            if img_data.startswith('http'):
                # Use the session with a longer timeout
                r = session.get(img_data, timeout=TIMEOUT_VAL)
                if r.status_code == 200:
                    Image.open(io.BytesIO(r.content)).convert('RGB').save(save_path, "JPEG", quality=95)
                else:
                    return f"{img_id}: HTTP {r.status_code}"
            else:
                Image.open(img_data).convert('RGB').save(save_path, "JPEG", quality=95)

        # PIL or dict-of-bytes cases (usually less prone to timeout errors)
        elif hasattr(img_data, 'convert'):
            img_data.convert('RGB').save(save_path, "JPEG", quality=95)
        elif isinstance(img_data, dict) and 'bytes' in img_data:
            Image.open(io.BytesIO(img_data['bytes'])).convert('RGB').save(save_path, "JPEG", quality=95)

    except Exception as e:
        return f"{img_id}: {str(e)}"

    return None

# Run an "exhaustive sweep"
print(f"Sweeping {len(unique_data_list)} images (especially the 1312 previously failed images)...")
with ThreadPoolExecutor(max_workers=NUM_THREADS) as executor:
    results = list(tqdm(executor.map(save_and_verify, unique_data_list), total=len(unique_data_list)))

# Stats
final_count = len([f for f in os.listdir(OUTPUT_IMAGES_DIR) if f.endswith('.jpg')])
print(f"\nStatus: {final_count}/6212 images.")

if final_count < 6212:
    failed = [r for r in results if r is not None]
    print(f"Still {len(failed)} images failed to download. Try running the script again in a few minutes.")
else:
    print("Great! You now have 100% of the data.")
